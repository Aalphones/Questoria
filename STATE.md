...
**Aktiver Plan:** [docs/planning/2026-08-19_ui-umbau-vollbild/README.md](docs/planning/2026-08-19_ui-umbau-vollbild/README.md) — **Phase 3 von 5 (Etappen- und Ortskarte im Vollbild)**, Rating standard.

**Nächster Schritt:** Phase 3 umsetzen — `map-canvas.scss` bekommt die Einpass-Regel (Fläche behält 16:9, passt sich in die Bühne ein, kein Beschneiden), dann `timeline` und `map` darauf verdrahten. Vorher: die Phase-3-Findings in [FINDINGS.md](docs/planning/2026-08-19_ui-umbau-vollbild/FINDINGS.md) lesen — `.map` und `.timeline` tragen aus Phase 1 ein übergangsweises `overflow: auto` und eine `90rem`-Breitenbremse, beides muss dort raus.

**Phase 1 und 2 sind fertig (19.08.2026), beide ungesehen.** Phase 1: die App hat jetzt eine Bühne, `100dvh`, die Seite rollt nie mehr, jeder Screen erbt die Höhe. Phase 2: die Planetenkarte füllt die Bühnenhöhe, wächst in die Breite und rollt waagerecht, wenn mehr Welten kommen — Content-Feld und Verdrahtung für Wege zwischen Welten gab es schon, gebaut wurde nur die Größenlogik. Build und Lint grün nach beiden Phasen — das heißt bei Layout gar nichts. Prüflisten für den Bildschirm stehen in den Report-Backs von [phase-1-buehne.md](docs/planning/2026-08-19_ui-umbau-vollbild/phase-1-buehne.md) und [phase-2-planetenkarte.md](docs/planning/2026-08-19_ui-umbau-vollbild/phase-2-planetenkarte.md). Wackligste Stellen: das Profilmenü in der Kopfleiste (Phase 1) und das CSS-Auto-Sizing der Planetenkarte aus `aspect-ratio` (Phase 2, moderner Browser-Mechanismus, ungetestet am Bildschirm).
