...
**Aktiver Plan:** [docs/planning/2026-08-19_ui-umbau-vollbild/README.md](docs/planning/2026-08-19_ui-umbau-vollbild/README.md) — **alle 5 Phasen fertig, erste Abnahme am Bildschirm gelaufen (19.08.2026), drei Befunde behoben, zweite Abnahme offen.**

**Die erste Abnahme hat den Grundfehler des ganzen Umbaus gefunden:** Die Bühne hat nie funktioniert. Angular hängt den Screen als **Geschwister nach** dem `<router-outlet>` ein, das Outlet-Element bleibt im DOM stehen — damit hatte das Bühnen-Grid zwei Elemente statt einem. Das leere Outlet belegte die einzige Vollhöhen-Zeile, der Screen rutschte in eine Zeile darunter und wurde unten abgeschnitten. Sichtbar als leere Fläche oben und einer Kopfleiste, die in der Bildmitte (Lernstufen) oder ganz unten (Etappenkarte) klebte. Behoben mit einer Zeile in `app.scss` (`.stage > router-outlet { display: none; }`). Build und Lint waren die ganze Zeit grün — bei Layout heißt das nichts, jetzt belegt.

**Dazu die Planetenkarte umgebaut:** Sie passt sich jetzt ein wie Etappen- und Ortskarte, steht mittig und hat einen Luftring, in den Knoten am Rand mit ihrem Namensschild ragen dürfen (die Pokémon-Kachel bei `y: 78` tat genau das). Der frei bleibende Bildschirmrand trägt dieselbe Karte ein zweites Mal, weichgezeichnet und abgedunkelt — so füllt der Hintergrund den ganzen Schirm, ohne dass ein Zuschnitt die Weltkoordinaten von ihren Landmarken wegschiebt. 🟡 **Abweichung:** Das waagerechte Rollen bei mehr Welten (Phase 2, AK 6) ist damit entfallen — es widerspricht der Vorgabe „Planeten dürfen nicht hinauslaufen". Festgehalten in `docs/design/README.md` Punkt 12 und in FINDINGS.md.

**Nächster Schritt:** Zweite Abnahme im Browser. Zuerst die drei gemeldeten Stellen: Planetenkarte (Welt vollständig im Bild, Hintergrund randlos), Lernstufen-Auswahl (Kopfleiste oben, Trainerkarten mittig darunter), Etappenkarte (bedienbar, Kopfleiste oben). Da der Outlet-Fehler **jeden** Screen betraf, sind die Abnahmen von Phase 1–5 damit hinfällig und laufen komplett neu — die 8 finalen Abnahmekriterien der README, Screen für Screen.

🟡 `main-hub.scss` liegt jetzt 58 Byte über dem 4-kB-Stylesheet-Budget des Builds (Warnung, kein Fehler). Budget bewusst nicht angehoben; vier andere Dateien reißen dasselbe Budget seit Längerem um bis zu 3,8 kB.
