...
9. **Jede Welt legt ihren Bildstil fest, bevor das erste Bild entsteht** — als `art_style` in `world_config.json`, ein englischer Satz, der wörtlich in jeden Bild-Prompt kopiert wird. Eine Welt braucht 30 bis 60 Bilddateien; ohne festgeschriebenen Stilsatz bekommt sie ebenso viele Handschriften, und das fällt erst bei Datei vierzig auf. Der Satz ist Pflichtfeld im Schema und Teil des Welt-Bauprompts — wer eine Welt anlegt und ihn weglässt, hat die Welt nicht fertig angelegt.
10. **Kein `@switch` über Eventtypen im Ablauf-Gerüst** — der Event Loader wählt die Komponente über `ngComponentOutlet` und eine Typ-Tabelle. Ein Sonderfall im Gerüst nimmt der Engine genau die Eigenschaft, für die sie gebaut ist.
