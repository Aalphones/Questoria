...
|---|---|---|
| App-Hülle (Bühne) | `app.html`, `app.scss` | Das `<main class="stage">` um den `<router-outlet />`: die einzige Stelle der App, die eine Bildschirmhöhe kennt (`--size-stage-block`, `100dvh`). Die Seite selbst rollt nie; jeder Screen erbt die Höhe mit `100%` und teilt sie selbst in Kopfleiste + Spielfläche ([ADR-017](decisions/017-vollbild-doktrin.md)). **Falle:** Angular hängt den Screen als Geschwister **nach** dem `<router-outlet>` ein — das Outlet-Element bleibt im DOM und wäre sonst selbst ein Grid-Element, deshalb `display: none` darauf |
| Main-Hub | `features/main-hub/` | Zwei Screens: Planetenkarte (Welten als Knoten auf der Kartenfläche, `theme-card/` als Weltknoten, `ui/map-panel/` mit „Weiterspielen", Erfolge-Panel mit allen Erfolgen aller installierten Welten) und `level-select/` mit der Lernstufen-Auswahl (`difficulty-picker/`) |
| Timeline | `features/timeline/` | Etappenkarte pro Welt, Fortschrittsmarkierung mit Sternen, `ui/map-panel/` mit „Fortschritt zurücksetzen". Vier Sichtbarkeits-Zustände pro Etappe (erkundet/aktuell/entdeckt/unbekannt, ADR-024) — unabhängig von `ProgressState` |
| Map | `features/map/` | Interaktive Ortskarte pro Arc — Orte als Punkte, Routen, Kompassrose, `ui/map-panel/` mit dem Zwischenstand der Karte. Dieselben vier Sichtbarkeits-Zustände wie die Timeline (ADR-024) |
| Event Engine | `features/episode/` | Spielt die Eventliste einer Episode ab: Ablauf-Gerüst (`episode.ts`), Laufzustand inkl. Lauf-Startwert (`episode-run.ts`), Event Loader + Typ-Prüfungen (`event-type-map.ts`), Auflösung ausgelagerter Konfigurationen inkl. Pool-/Generator-Fassungen (`resolve-event-config.ts`, ADR-016), Ergebnis-Einsammlung, Wiedereinstiegs-Frage nach Abbruch (`resume-prompt/`). Kein `@switch` über Eventtypen |
...
| Kartenfläche | `ui/map-canvas/` | Gemeinsames Bauteil von Planeten-, Etappen- und Ortskarte: kachelbasierte, fortschrittsgesteuerte Kartenfläche mit offenem Koordinatensystem (1024×1024-Kacheln, nur freigeschaltete werden geladen), Routenlinien, Ziehen und Zoomen ohne Fremdbibliothek ([ADR-019](decisions/019-panzoom-ohne-bibliothek.md)). Bezugsfläche ist die freigeschaltete Fläche **plus ein Kachelrand Nebel**, beschnitten auf die Gesamtkarte; der Maßstab wird direkt geklemmt — herausgezoomt passt die ganze Fläche hinein, der Startzustand füllt randlos, hinein geht es bis zur nativen Kachelgröße ([ADR-022](decisions/022-kartenausschnitt-und-zoomboden.md)). Gesperrte Kacheln zeigen keine eigenen Elemente mehr — der Nebel ist eine SVG-Schicht in Weltkoordinaten mit einer weichgezeichneten Lochmaske über den freigeschalteten Kacheln, zwei Lagen (Kern/Hof), unter den Knoten und über Kacheln/Routen ([ADR-023](decisions/023-nebel-als-eigene-schicht.md)). `map-point/` setzt ein beliebiges Kind auf eine Weltposition (Pixel) und hält über `--map-inverse-scale` seine Bildschirmgröße gegen den Zoom. Oben rechts eine Minikarte (Kachelform, Knotenpunkte, Ausschnitt-Rechteck aus `translateX/Y`, reine Anzeige mit `aria-live`-Ansage statt Bedienelement), unter den Zoom-Knöpfen ein Legenden-Dialog mit den vier Sichtbarkeits-Zuständen — beide weichen unter `$map-narrow` |
| Kartenpanel | `ui/map-panel/` | Gemeinsames Info-Panel aller drei Kartenebenen: Tag, Titel, Hinweis, optional Zwischenstand mit Balken (`role="progressbar"`), darunter projizierter Inhalt (z. B. „Weiterspielen"). Aufklapp-Zustand steckt im Bauteil, nicht im Screen; unter `$map-narrow` eine Leiste am unteren statt oberen Rand |
...
| Zentrale Services | `services/game-state.service.ts`, `services/content.service.ts`, `services/progress.service.ts`, `services/progress.rules.ts`, `services/run-store.service.ts`, `services/variation.ts`, `services/variant-history.service.ts`, `services/savegame.service.ts`, `services/achievement.service.ts`, `services/achievement.rules.ts`, `services/statistics.service.ts`, `services/narration.service.ts`, `services/auth.service.ts`, `services/profile.service.ts`, `services/legacy-progress-import.ts` | Aktive Welt/Profil-ID/Lernstufe (Profil-ID im Browser-Speicher, überlebt Neuladen), JSON-Content lesen, Fortschritt aus dem Spielstand des Profils + Freischaltregeln als reine Funktionen (ADR-009 löst ADR-006 ab; `ProgressService` trägt seit dem Kachel-Plan auch die Kachel-Freischaltung — `revealedTileIds`/`syncRevealedTiles`, abgeleitet vereinigt mit einer gespeicherten Hochwassermarke, ADR-020), `nodeRevealStates` in `progress.rules.ts` leitet zusätzlich ab, was auf der Karte *sichtbar* ist, unabhängig davon, was *spielbar* ist (ADR-024), der eine angefangene Lauf im Spielstand derselben Welt inkl. Startwert-Vergabe, das Variationssystem als reine Zufallsfunktionen (`seededRandom`, `shuffle`, `selectFromPool`, `generateInteger`, `resolveTemplate`, `satisfiesConstraints` — ADR-016) und die App-Neustart-feste Meidungsliste zuletzt benutzter Pool-Fassungen je Aufgabe, Speichern/Laden mit Puffer im Browser, einmalige Übernahme des alten Browser-Stands, erreichte Erfolge mit demselben Puffer + Auswertung der Content-Bedingungen als reine Funktionen (ADR-010), über alle Läufe wachsende Statistiken mit demselben Puffer, aber als Warteschlange additiver Zuwächse statt eines ersetzenden Stands (Plan Phase 8), Vorlesemodus + Sprachausgabe, angemeldeter Benutzer (Sitzung selbst bleibt im HttpOnly-Cookie), Profilliste des Accounts (laden, anlegen, ändern, löschen, wählen). **`ContentService` ist die einzige Ladestelle für Content** — dort hängt später der Offline-Cache (Meilenstein 6) |
...
`ui/map-panel/`, `ui/hud/` (inkl. Modus-Umschalter, Ton-Knopf und echter Profil-Chip mit Menü), `ui/content-error/`,
...
| `data/audio/engine/` | Die zwei festen Ansagen der Engine (Fortsetzen-Dialog, Erfolgs-Nachricht) — weltunabhängig, deshalb neben `themes/` statt darin. Erzeugt von `voice-tools/generate_engine_lines.py`, geholt über `ContentService.engineAudioUrl()` |
