...
4. JWT-Middleware manuell gegengeprüft: ein kurzes Wegwerf-Skript (`php -r` bzw. `backend/scratch.php`, **nicht committen**) signiert einen Token und liest ihn wieder aus, ein manipulierter Token wirft `ApiException`. Ergebnis im Report-Back festhalten, Skript danach löschen.
...
- [ ] `backend/composer.json` anlegen (PSR-4 `App\` → `src/`):
      ```json
      {
      ...
      ```
- [ ] `backend/.php-cs-fixer.php` — identisches Regelwerk wie promptigofant (`@PSR12`, `declare_strict_types`, short array syntax, `single_quote`, `ordered_imports`, `trailing_comma_in_multiline`), `Finder` über `src/` und `public/`
- [ ] `backend/.env.example`:
      ```
      APP_ENV=local
      DB_HOST=127.0.0.1
