...
| Wort-Bild-Paare (`word_match`) | Die Aufgabe, die Lesen misst statt Wissen: drei bis vier Bilder und ebenso viele Wortkarten, das Kind legt zusammen, was zusammengehört. Jedes Wort muss gelesen werden, und keins wird vorgelesen — auch im Vorlesemodus nicht. Der Stern hängt an der ganzen Aufgabe: ein einziger Fehlgriff kostet ihn. |
| Bildstil (`art_style`) | Pflichtfeld jeder Welt in `world_config.json`: ein englischer Satz, der **wörtlich** in jeden Bild-Prompt dieser Welt kopiert wird. Er ist der Grund, warum 52 Bilder aus vier Läufen wie ein Satz aussehen — nicht Geschmack, sondern die einzige Klammer über Modelle und Sitzungen hinweg. |
| Bestellliste | Die vollständige Aufzählung aller Bilddateien einer Welt (Zielpfad, Maß, Format, Motiv), bevor das erste erzeugt wird. Sie ist der Zähler beim Abarbeiten und der Grund, warum „rund 25 Bilder" bei `pokemon_lesen` in Wahrheit 52 waren. |
...
| Statistik | Vier Zahlen pro Welt und Profil, die über **alle** Läufe hinweg wachsen (geschaffte Aufgaben, richtige/falsche Antworten, Spielzeit) — Tabelle `statistics`, addiert statt gesetzt. Abgrenzung zum **Lauf**: `EpisodeRun` zählt nur den gerade gespielten Durchgang, die Statistik ist die dauerhafte Summe daraus. Ein `run_id`-Feld schützt gegen Doppelzählung, falls derselbe abgeschlossene Lauf nach totem Server ein zweites Mal ankommt. |
